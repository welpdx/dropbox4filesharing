...
// @icon         https://i.pinimg.com/736x/c9/95/f6/c995f65a9eb964a934fa3d5b7bef4ebf.jpg
// @grant       none
// @version     1.1
// @author      Ace
...
function startSolving() {
    ...
    intervalId = setInterval(solve, randomInt(800,1400));
}
...
function solve() {
    ...
    if (selNext.length === 1) {
                ...
                if (document.querySelectorAll('[data-test="challenge-tap-token-text"]').length == document.querySelectorAll('[data-test="challenge-tap-token"]').length) {
                    window.sol.pairs.forEach((pair) => {
                        for (let i = 0; i < nl.length; i++) {
                            ...
                            if (
                                ( //used to be pair.learningToken and pair.fromToken
        ...
        //ad for duo +
        if (document.querySelectorAll('[data-test="plus-no-thanks"]').length > 0) {
            var node = document.querySelectorAll('[data-test="plus-no-thanks"]');
            node[0].click();
        }
        //legendary maybe later
        if (document.querySelectorAll('[data-test="maybe-later"]').length > 0) {
            var l8ter = document.querySelectorAll('[data-test="maybe-later"]');
            l8ter[0].click();
        }
        //final crown
        //if (document.querySelectorAll('[data-test="final-level-promo"]').length > 0) {
            //var crown = document.querySelectorAll('[data-test="maybe-later"]');
            //l8ter[0].click();
        //}
        // Continue
